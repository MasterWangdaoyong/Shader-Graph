Shader "JianpingWang/Test/Test_SceneSSS" {
	Properties {
		...
		_ThicknessScale ("_ThicknessScale", float) = 1
		_ThicknessScale2 ("_ThicknessScale2", float) = 1
	...
	SubShader {
		...
		float _Scale, _Power, _Distortion , _ThicknessScale, _ThicknessScale2;
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			o.Alpha = tex2D(_Thickness, IN.uv_MainTex).r * _ThicknessScale;
			o.Gloss = tex.a;
		...
		{		
			...
			//Main
			float transDot2 = pow ( max (0, dot (-viewDir, -transLightDir ) ), _Power ) * _Scale;
			fixed3 transLight2 = (atten ) * ( transDot2 ) * s.Alpha * _SubColor.rgb;
			...
			fixed4 c;
			c.rgb = lerp(transAlbedo2 + transAlbedo + diffAlbedo * _ThicknessScale2, transAlbedo + diffAlbedo * _ThicknessScale2, 0.8);

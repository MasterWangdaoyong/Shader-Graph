...
Shader "Dodjoy/Effect/LineRepeat"
{
   Properties 
   {
	...
	Lighting Off
      ...
      {   
         ...
         float4 frag ( vertexOutput input ):Color
         {
            ...
            float4 rtc =  tex2D(_MainTex, float2(-(input.tex.x* ceil(_Float) + -_Time.y * _Float2),input.tex.y));
			rtc += _TintColor;
			//实现隔一段时间闪一下的效果
			float num = (fmod(_Time.y,_FlashTime) / (_FlashTime / 2)); 
			rtc.a = abs(step(1,num)*2 - num) * rtc.a;
			//右边Alpha渐变
			rtc.a *= clamp((input.tex.x / _Float3),0,1);
			//左边Alpha渐变
			rtc.a *= clamp(((1 - input.tex.x) / _Float3),0,1);
            return rtc;
         }
